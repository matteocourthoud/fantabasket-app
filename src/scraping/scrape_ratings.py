...
def extract_dunkest_table(driver):
...
def scrape_ratings(url: str):
    ...
    while True:
        df_current_ratings = pd.concat([df_current_ratings, extract_dunkest_table(driver)])
...
def update_ratings(data_dir: str, season: int):
    ...
    df_current_ratings = scrape_ratings(url=url)[["name", "rating"]]
...
def update_initial_ratings(data_dir: str, season: int) -> pd.DataFrame:
